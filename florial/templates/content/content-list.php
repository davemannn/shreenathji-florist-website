...
<div class="list-post">
    <article id="post-<?php esc_attr(the_ID()); ?>" <?php post_class(); ?>>
		<div class="post-content">
			...
			<div class="entry-meta-head">
				<?php if (florial_get_config('archives-author')) { ?>
					<div class="entry-author">
						<span class="entry-meta-link"><?php echo esc_html__("By : ",'florial') ?><?php echo the_author_posts_link(); ?></span>
					</div>
				<?php } ?>
			...
			<a class="read-more" href="<?php echo esc_url(the_permalink()) ?>"><?php echo esc_html__("Learn More","florial"); ?></a>
    	</div>
		...
		<?php elseif( $format == 'video' || $format == 'audio' ) : ?>
			...
			<div class="entry-thumb single-thumb">
				<a class="post-thumbnail" href="<?php esc_url(the_permalink()); ?>" title="<?php echo the_title_attribute(); ?>">
					<?php the_post_thumbnail( 'florial-full-width', array( 'alt' => get_the_title() ) );	?>      
				</a> 
			</div>
			<?php } ?>
						...
						<div class="slick-carousel" data-columns4="1" data-columns3="1" data-columns2="1" data-columns1="1" data-columns="1" data-nav="true">
								<?php
									if($ids){
										...
										foreach ( $ids as $i => $id ){ ?>
											<div class="item">
													<?php echo wp_get_attachment_image($id, 'florial-full-width'); ?>
											</div>
										<?php }	
									}
								?>
