<?php
define('florial_version','1.0'); 
define('url_demo','https://wpbingosite.com/wordpress/florial');
...
function florial_custom_css() {
	...
	if (!is_admin()) {
		...
		wp_add_inline_style( 'florial-style-template', implode($custom_css) );
	}
}
add_action('wp_enqueue_scripts', 'florial_custom_css' );
function florial_custom_js() {
	if (!is_admin()) {
		wp_enqueue_script( 'florial-script', get_template_directory_uri() . '/js/functions.js', array( 'jquery'), null, true );
		wp_localize_script( 'florial-script', 'florial_ajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
		...
		wp_add_inline_script( 'florial-script', $custom_js);
	}
}
add_action('wp_enqueue_scripts', 'florial_custom_js' );