 ...
 */
export function DeliveryEstimate({ midnightCutoffHour }: { midnightCutoffHour: number }) {
  const cutoff12h =
  ...
  return (
    <div className="border-border flex flex-col gap-3 rounded-xs border p-4 text-sm">
      <div className="flex items-center gap-2.5">
        ...
        <span>Order anytime for same-day (2-4 hour) delivery in Vadodara.</span>
      </div>
      <div className="flex items-center gap-2.5">
        <Moon className="text-brand size-4 shrink-0" aria-hidden="true" />
        <span>
          Order before {cutoff12h} for midnight delivery tonight — choose your slot at checkout.
        </span>
      </div>
    </div>
  );
